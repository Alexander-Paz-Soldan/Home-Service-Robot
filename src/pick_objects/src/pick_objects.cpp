...
int main(int argc, char** argv){
  ...
  ROS_INFO("Pick object: Sending goal");
  ac.sendGoal(goal);
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
   {
    ROS_INFO("Message from pick_object: Goal has been reached");
    ros::Duration(5).sleep(); // sleep for 5 and go to next goal
   }
  else
    ROS_INFO("Message from pick object: Somehow the first goal could not be reached");
